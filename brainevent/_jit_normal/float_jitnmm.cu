...
/*
 * float_jitnmm.cu -- dense-matrix light-RNG backends.
 *
 * jitnmm_notrans/trans_f32 use the AW-T4 MM random stream and match CSR
 * matrix_mode="mm".  jitnmm_mv_notrans/trans_f32 use the MV random stream
 * column-by-column and are only used by vmap(jitnmv).
 */
...
static void launch_mv_notrans_f32(
    ...
    if (row_warp_blocks > 2147483647 || n_chunks > 65535 || n > 65535) {
        fprintf(stderr,
                "jitnmm_mv_notrans_f32 grid overflow: row_warp_blocks=%d n_chunks=%d n=%d\n",
...
static void launch_mv_trans_f32(
    ...
    if (row_warp_blocks > 2147483647 || n_chunks > 65535 || n > 65535) {
        fprintf(stderr,
                "jitnmm_mv_trans_f32 grid overflow: row_warp_blocks=%d n_chunks=%d n=%d\n",
                row_warp_blocks, n_chunks, n);
        abort();
    }
    dim3 blocks((unsigned int)row_warp_blocks, (unsigned int)n_chunks, (unsigned int)n);
...
static void launch_mm_notrans_f32(
    ...
    if (row_group_blocks > 2147483647 || n_chunks > 65535 || n > 65535) {
        fprintf(stderr,
                "jitnmm_notrans_f32 grid overflow: row_group_blocks=%d n_chunks=%d n=%d\n",
...
static void launch_mm_trans_f32(
    ...
    if (row_group_blocks > 2147483647 || n_chunks > 65535 || n > 65535) {
        fprintf(stderr,
                "jitnmm_trans_f32 grid overflow: row_group_blocks=%d n_chunks=%d n=%d\n",
                row_group_blocks, n_chunks, n);
        abort();
    }
    dim3 blocks((unsigned int)row_group_blocks, (unsigned int)n_chunks, (unsigned int)n);
...
// @BE jitnmm_mv_notrans_f32
void jitnmm_mv_notrans_f32(
    const BE::Tensor w_loc,
    const BE::Tensor w_scale,
    const BE::Tensor clen,
    const BE::Tensor seed,
    ...
    launch_mv_notrans_f32(w_loc, w_scale, clen, seed, B, output, m, k, n, chunk_size, stream);
}

// @BE jitnmm_mv_trans_f32
void jitnmm_mv_trans_f32(
    const BE::Tensor w_loc,
    const BE::Tensor w_scale,
    const BE::Tensor clen,
    const BE::Tensor seed,
    ...
    launch_mv_trans_f32(w_loc, w_scale, clen, seed, B, output, m, k, n, chunk_size, stream);
}

// @BE jitnmm_notrans_f32
void jitnmm_notrans_f32(
    const BE::Tensor w_loc,
    const BE::Tensor w_scale,
    const BE::Tensor clen,
    const BE::Tensor seed,
    ...
    launch_mm_notrans_f32(w_loc, w_scale, clen, seed, B, output, m, k, n, chunk_size, stream);
}

// @BE jitnmm_trans_f32
void jitnmm_trans_f32(
    const BE::Tensor w_loc,
    const BE::Tensor w_scale,
    const BE::Tensor clen,
    const BE::Tensor seed,
    ...
    launch_mm_trans_f32(w_loc, w_scale, clen, seed, B, output, m, k, n, chunk_size, stream);
}
...
#define FFI_JITNMM(NAME, KERNEL, SFX, WEIGHT_T, ROW_GROUPS, OUT_ROWS)            \
void NAME##SFX(                                                                  \
    const BE::Tensor w_loc,                                                      \
    const BE::Tensor w_scale,                                                    \
    const BE::Tensor clen,                                                       \
...
// @BE jitnmm_mv_notrans_f64
FFI_JITNMM(jitnmm_mv_notrans, mv_notrans, _f64, double, 1, m)
// @BE jitnmm_mv_notrans_f16
FFI_JITNMM(jitnmm_mv_notrans, mv_notrans, _f16, __half, 1, m)
// @BE jitnmm_mv_notrans_bf16
FFI_JITNMM(jitnmm_mv_notrans, mv_notrans, _bf16, __nv_bfloat16, 1, m)

// @BE jitnmm_mv_trans_f64
FFI_JITNMM(jitnmm_mv_trans, mv_trans, _f64, double, 1, k)
// @BE jitnmm_mv_trans_f16
FFI_JITNMM(jitnmm_mv_trans, mv_trans, _f16, __half, 1, k)
// @BE jitnmm_mv_trans_bf16
FFI_JITNMM(jitnmm_mv_trans, mv_trans, _bf16, __nv_bfloat16, 1, k)

// @BE jitnmm_notrans_f64
FFI_JITNMM(jitnmm_notrans, mm_notrans, _f64, double, AW_T4_GROUPS_PER_WARP, m)
// @BE jitnmm_notrans_f16
FFI_JITNMM(jitnmm_notrans, mm_notrans, _f16, __half, AW_T4_GROUPS_PER_WARP, m)
// @BE jitnmm_notrans_bf16
FFI_JITNMM(jitnmm_notrans, mm_notrans, _bf16, __nv_bfloat16, AW_T4_GROUPS_PER_WARP, m)

// @BE jitnmm_trans_f64
FFI_JITNMM(jitnmm_trans, mm_trans, _f64, double, AW_T4_GROUPS_PER_WARP, k)
// @BE jitnmm_trans_f16
FFI_JITNMM(jitnmm_trans, mm_trans, _f16, __half, AW_T4_GROUPS_PER_WARP, k)
// @BE jitnmm_trans_bf16
FFI_JITNMM(jitnmm_trans, mm_trans, _bf16, __nv_bfloat16, AW_T4_GROUPS_PER_WARP, k)

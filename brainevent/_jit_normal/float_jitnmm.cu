 ...
 *
 * jitumm_gather/scatter_f32 use the AW-T4 MM random stream and match CSR
 * matrix_mode="mm".  jitumm_mv_gather/scatter_f32 use the MV random stream
...
static void launch_mv_gather_f32(
    ...
    if (row_warp_blocks > 2147483647 || n_chunks > 65535 || n > 65535) {
        fprintf(stderr,
                "jitnmm_mv_gather_f32 grid overflow: row_warp_blocks=%d n_chunks=%d n=%d\n",
...
static void launch_mv_scatter_f32(
    ...
    if (row_warp_blocks > 2147483647 || n_chunks > 65535 || n > 65535) {
        fprintf(stderr,
                "jitnmm_mv_scatter_f32 grid overflow: row_warp_blocks=%d n_chunks=%d n=%d\n",
                row_warp_blocks, n_chunks, n);
        abort();
    }
    dim3 blocks((unsigned int)row_warp_blocks, (unsigned int)n_chunks, (unsigned int)n);
...
static void launch_mm_gather_f32(
    ...
    if (row_group_blocks > 2147483647 || n_chunks > 65535 || n > 65535) {
        fprintf(stderr,
                "jitnmm_gather_f32 grid overflow: row_group_blocks=%d n_chunks=%d n=%d\n",
...
static void launch_mm_scatter_f32(
    ...
    if (row_group_blocks > 2147483647 || n_chunks > 65535 || n > 65535) {
        fprintf(stderr,
                "jitnmm_scatter_f32 grid overflow: row_group_blocks=%d n_chunks=%d n=%d\n",
                row_group_blocks, n_chunks, n);
        abort();
    }
    dim3 blocks((unsigned int)row_group_blocks, (unsigned int)n_chunks, (unsigned int)n);
...
// @BE jitnmm_mv_gather_f32
void jitnmm_mv_gather_f32(
    const BE::Tensor w_loc,
    const BE::Tensor w_scale,
    const BE::Tensor clen,
    const BE::Tensor seed,
    ...
    launch_mv_gather_f32(w_loc, w_scale, clen, seed, B, output, m, k, n, chunk_size, stream);
}

// @BE jitnmm_mv_scatter_f32
void jitnmm_mv_scatter_f32(
    const BE::Tensor w_loc,
    const BE::Tensor w_scale,
    const BE::Tensor clen,
    const BE::Tensor seed,
    ...
    launch_mv_scatter_f32(w_loc, w_scale, clen, seed, B, output, m, k, n, chunk_size, stream);
}

// @BE jitnmm_gather_f32
void jitnmm_gather_f32(
    const BE::Tensor w_loc,
    const BE::Tensor w_scale,
    const BE::Tensor clen,
    const BE::Tensor seed,
    ...
    launch_mm_gather_f32(w_loc, w_scale, clen, seed, B, output, m, k, n, chunk_size, stream);
}

// @BE jitnmm_scatter_f32
void jitnmm_scatter_f32(
    const BE::Tensor w_loc,
    const BE::Tensor w_scale,
    const BE::Tensor clen,
    const BE::Tensor seed,
    ...
    launch_mm_scatter_f32(w_loc, w_scale, clen, seed, B, output, m, k, n, chunk_size, stream);
}

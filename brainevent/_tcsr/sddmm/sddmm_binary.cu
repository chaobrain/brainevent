...
#pragma unroll
                    for (int word = 0; word < kWordsPerPhase; ++word)
                    {
                        ...
                        while (active_bits != 0)
                        {
                            ...
                            partial += ct[
                                static_cast<size_t>(batch_index) * cols + col];
...
template <int Batch, typename EventT, typename IndptrT>
    ...
    cudaStream_t stream)
{
    ...
    for (int macro_tile = 0; macro_tile < macro_tiles; ++macro_tile)
    {
        ...
        {
            persistent_macrotile_phase_kernel<Batch, IndptrT>
    ...
    int64_t stream)
{
    const BE::DType expected_event_dtype =
        std::is_same<EventT, bool>::value ? BE::DType::Bool : BE::DType::Float32;
    ...
    BE_CHECK(ct.dtype() == BE::DType::Float32 &&
        ...
        << "persistent macro-tile expects float32 CT and output";
...
#define LAUNCH_BATCH(BATCH)                                                    \
    BE_DISPATCH_CSR_INDPTR(indptr.dtype(), IndptrT, {                          \
        launch_fixed_batch<BATCH, EventT, IndptrT>(                            \
...
void tcsr_sddmm_dweight_binary_f32_bool_t(
    ...
    int64_t stream)
{
    launch_persistent_macrotile<bool>(
...
void tcsr_sddmm_dweight_binary_f32_float_t(
    ...
    int64_t stream)
{
    launch_persistent_macrotile<float>(
        event, ct, local_targets, indptr, tile_offsets, dweight, masks, stream);
}

...
__device__ __forceinline__ bool is_active(float value)
{
    return value != 0.0f;
}
...
template <int Batch, typename IndptrT>
    ...
    int phase)
{
    ...
    for (int64_t chunk = blockIdx.x; chunk < chunks; chunk += gridDim.x)
    {
        ...
        const int row_tasks = static_cast<int>(active_counts[
...
#pragma unroll
            for (int word = 0; word < kWordsPerPhase; ++word)
            {
                ...
                while (active_bits != 0)
                {
                    ...
                    const float lane_B_value = group_lane == 0
                        ? B[static_cast<size_t>(row) * Batch + batch_index]
                        : 0.0f;
                    const float B_value =
...
#pragma unroll
                         ...
                         ++subtile)
                    {
                             ...
                             relative0 += 2 * kGroupSize)
                        {
                            ...
                            const float value0 = B_value * ct[ct_offset + col0];
                            const float value1 = valid1
    ...
    cudaStream_t stream)
{
    ...
    build_phase_masks_kernel<Batch>
    ...
    for (int macro_tile = 0; macro_tile < macro_tiles; ++macro_tile)
    {
        for (int phase = 0; phase < kPhases; ++phase)
        {
            b_bit_driven_kernel<Batch, IndptrT>
...
void launch_sddmm_float(
    ...
    int64_t stream)
{
    BE_CHECK(B.dtype() == BE::DType::Float32 &&
             ct.dtype() == BE::DType::Float32 &&
             dweight.dtype() == BE::DType::Float32)
        << "Batch-N float SDDMM expects float32 dense values and output";
...
#define LAUNCH_BATCH(BATCH)                                                    \
    BE_DISPATCH_CSR_INDPTR(indptr.dtype(), IndptrT, {                          \
        launch_fixed_batch<BATCH, IndptrT>(                         \
            B, ct, local_targets, indptr, tile_offsets, dweight, masks,         \
            first_phase, active_counts, active_rows, rows, cols, cuda_stream);  \
    })
    switch (batch)
    {
    case 4: LAUNCH_BATCH(4); break;
    case 8: LAUNCH_BATCH(8); break;
    case 16: LAUNCH_BATCH(16); break;
    case 32: LAUNCH_BATCH(32); break;
    case 64: LAUNCH_BATCH(64); break;
    case 128: LAUNCH_BATCH(128); break;
    case 256: LAUNCH_BATCH(256); break;
    case 512: LAUNCH_BATCH(512); break;
    default:

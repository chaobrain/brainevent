...
namespace
{
    ...
    __global__ void compact_nonzero_float_rows_kernel(
        const ValueT *__restrict__ event,
        ...
        int64_t rows)
    {
        ...
        const bool active = row < rows && event[row] != ValueT(0);
...
#pragma unroll
        ...
        int macro_tile)
    {
             ...
             position += task_stride)
        {
            ...
            const ValueT event_value = event[row];
...
#pragma unroll
    ...
    void launch_single_float_sddmm(
        ...
        int64_t stream)
    {
        BE_CHECK(event.dtype() == TensorDType<ValueT>::value &&
                 ct.dtype() == TensorDType<ValueT>::value &&
                 dweight.dtype() == TensorDType<ValueT>::value)
            << "single-batch float TileCSR SDDMM value dtype mismatch";
        ...
        compact_nonzero_float_rows_kernel<ValueT>
            <<<static_cast<unsigned int>(compact_blocks64),
               kThreadsPerBlock,
               0,
               cuda_stream>>>(
            event.data_ptr<const ValueT>(),
        ...
        BE_DISPATCH_CSR_INDPTR(indptr.dtype(), IndptrT, {
            ...
            {
                single_float_sddmm_panel_kernel<ValueT, IndptrT>
                    <<<persistent_blocks,
                       kThreadsPerBlock,
                       0,
                       cuda_stream>>>(
...
void tcsr_sddmv_dweight_float_f32_t(
    ...
    int64_t stream)
{
    launch_single_float_sddmm<float>(
        event,
        ct,
        local_targets,
        indptr,
...
// @BE tcsr_sddmv_dweight_float_f64_t arg arg arg arg arg ret ret ret stream
void tcsr_sddmv_dweight_float_f64_t(
    const BE::Tensor event,
    const BE::Tensor ct,
    const BE::Tensor local_targets,
    const BE::Tensor indptr,
    ...
    int64_t stream)
{
    launch_single_float_sddmm<double>(
        event,
        ct,
        local_targets,
        indptr,

...
def _reference_dense(weight, prob, seed, shape, corder, backend):
    """The (non-transposed) dense matrix that ``jits_to_csr`` materializes."""
    return jits(
        weight, prob, seed,
        shape=shape, transpose=False, corder=corder, backend=backend,
    )
...
class Test_Scalar_To_CSR:
    ...
    def test_to_csr_roundtrip(self, implementation, corder, shape):
        ...
        dense = _reference_dense(1.5, 0.2, 123, shape, corder, implementation)
    ...
    def test_to_csr_agrees_with_matrix_view(self, implementation, corder):
        # jits_to_csr reproduces exactly the matrix the class materializes.
        shape = (20, 30)
    ...
    def test_to_csr_units(self):
        ...
        dense = _reference_dense(weight, 0.2, 123, shape, True, None)
    ...
    def test_to_csr_structure_valid(self, corder):
        ...
        shape = (20, 30)
        csr = jits_to_csr(1.5, 0.2, 123, shape=shape, corder=corder, backend=None)
...
def test_to_csr_signature_matches_0_1_2():
    import inspect
    assert tuple(inspect.signature(jits_to_csr).parameters) == tuple(
        p.strip() for p in 'weight'.split(',')

...
data order as ``jitu_to_csr(..., matrix_mode="mv")``. Unlike a wrapper around
``tocsr().dt2t(...)``, the fill pass draws each uniform weight and multiplies by
...
1. the light JIT-uniform CSR count pass, which determines row/chunk offsets; and
...
def jitumv_dt2t(
    ...
    *,
    ...
    ``jitu_to_csr(..., matrix_mode="mv").data``. The output equals
...
# ---------------------------------------------------------------------- #
#
# The dt2t path deliberately reuses the light JIT-uniform CSR count pass instead
# of duplicating it here. This keeps the data-dependent ``nnz``/chunk-offset
# logic aligned with ``jitu_to_csr(..., matrix_mode="mv")``; the dedicated dt2t
...
Given the ``chunk_offsets`` produced from the light JIT-uniform CSR count pass,
this primitive walks the same deterministic random stream as
``jitu_to_csr(..., matrix_mode="mv")``. For every generated connection it writes

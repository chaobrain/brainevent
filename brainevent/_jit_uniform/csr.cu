...
__global__ void _fill_notrans##SFX##_kern( \
    ...
    while (local_j < chunk_width) { \
        ...
        indices[pos] = j; \
        float u01 = hash_uniform01(seed0, row, j); \
        data[pos] = WRITE_W(wlo + (ACC_T)u01 * range); \
        write += 1; \
        q += 1U + fast_bounded_u32(light_rng_next(&rng), cl - 1U); \
        local_j = (unsigned int)lane + 32U * q; \
    } \
...
#define DEFINE_COUNT_TRANS_WRAPPER(NAME, KERNEL, SFX) \
void NAME##SFX( \
    const BE::Tensor w_low, \
    const BE::Tensor w_high, \
    const BE::Tensor clen, \
...
#define DEFINE_COUNT_NOTRANS_WRAPPER(NAME, KERNEL, SFX) \
void NAME##SFX( \
    const BE::Tensor w_low, \
    const BE::Tensor w_high, \
    const BE::Tensor clen, \
...
#define DEFINE_FILL_TRANS_WRAPPER(NAME, KERNEL_BASE, SFX, WEIGHT_T) \
void NAME##SFX( \
    const BE::Tensor w_low, \
    const BE::Tensor w_high, \
    const BE::Tensor clen, \
...
#define DEFINE_FILL_NOTRANS_WRAPPER(NAME, KERNEL_BASE, SFX, WEIGHT_T) \
void NAME##SFX( \
    const BE::Tensor w_low, \
    const BE::Tensor w_high, \
    const BE::Tensor clen, \
...
DEFINE_COUNT_TRANS_WRAPPER(count_chunks_trans, _count_chunks_trans_f32_kern, _f64)
// @BE count_chunks_trans_f16
DEFINE_COUNT_TRANS_WRAPPER(count_chunks_trans, _count_chunks_trans_f32_kern, _f16)
// @BE count_chunks_trans_bf16
DEFINE_COUNT_TRANS_WRAPPER(count_chunks_trans, _count_chunks_trans_f32_kern, _bf16)

// @BE fill_trans_f64
DEFINE_FILL_TRANS_WRAPPER(fill_trans, _fill_trans, _f64, double)
// @BE fill_trans_f16
DEFINE_FILL_TRANS_WRAPPER(fill_trans, _fill_trans, _f16, __half)
// @BE fill_trans_bf16
DEFINE_FILL_TRANS_WRAPPER(fill_trans, _fill_trans, _bf16, __nv_bfloat16)

// @BE count_chunks_notrans_f64
DEFINE_COUNT_NOTRANS_WRAPPER(count_chunks_notrans, _count_chunks_notrans_f32_kern, _f64)
// @BE count_chunks_notrans_f16
DEFINE_COUNT_NOTRANS_WRAPPER(count_chunks_notrans, _count_chunks_notrans_f32_kern, _f16)
// @BE count_chunks_notrans_bf16
DEFINE_COUNT_NOTRANS_WRAPPER(count_chunks_notrans, _count_chunks_notrans_f32_kern, _bf16)

// @BE fill_notrans_f64
DEFINE_FILL_NOTRANS_WRAPPER(fill_notrans, _fill_notrans, _f64, double)
// @BE fill_notrans_f16
DEFINE_FILL_NOTRANS_WRAPPER(fill_notrans, _fill_notrans, _f16, __half)
// @BE fill_notrans_bf16
DEFINE_FILL_NOTRANS_WRAPPER(fill_notrans, _fill_notrans, _bf16, __nv_bfloat16)

...
#
# These helpers mirror ``update_csr_on_binary_pre`` / ``update_csr_on_binary_post``
# but operate on ``weight`` arrays stored in *column-major* (CSC) order.  They
# reuse the CSR plasticity primitives above by exploiting the structural
# symmetry between the two layouts:
#
...
#   reduces to ``update_csr_on_binary_pre`` on the transposed shape, with no
...
#   ``update_csr_on_binary_post``, which scatters the per-synapse updates back
...
def update_csc_on_binary_pre(
    ...
    *,
    ...
    :func:`update_csr_on_binary_post`, which scatters the per-synapse
    ...
    update_csr_on_binary_post : The CSR primitive this reuses.
...
def update_csc_on_binary_post(
    ...
    *,
    ...
    ``W.T``, the operation reduces to :func:`update_csr_on_binary_pre`
    ...
    update_csr_on_binary_pre : The CSR primitive this reuses.

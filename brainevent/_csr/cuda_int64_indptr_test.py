...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            yw2w_mod._csrmv_yw2w_cuda_kernel,
...
def test_slice_yw2w_and_plasticity_cuda_generators_accept_int64_indptr_without_real_cuda(monkeypatch):
    ffi_calls = []
    load_calls = []
    ...
    monkeypatch.setattr(yw2w_mod, "load_cuda_file", lambda path, name: load_calls.append((path, name)))
    monkeypatch.setattr(yw2w_mod.jax.ffi, "ffi_call", _recording_ffi_call(ffi_calls))
    ...
    with _jax_x64_enabled():
        ...
        yw2w_kernel = yw2w_mod._csrmv_yw2w_cuda_kernel(
            False,
            _shape(jnp.float32, (2,)),
            **_cuda_kwargs(indices_dtype=jnp.int32, indptr_dtype=jnp.int64),
        )
        yw2w_kernel(jnp.array([1.0]), jnp.array([2.0, 3.0]), indices, indptr)

 ...
 *   - ARAW scatter mode (transpose=True, batch-major input): output[j, indices[i,k]] +=
 *     weights[i,k] * is_active(matrix[j,i])
 *
 ...
 *   - ARAW aggregate-read / aggregate-write: same physical output layout as
 *     SRAW, but reads batch-major spike input [n_batch, n_pre].
 */
...
#define ARAW_FCNMM_WARP_THREADS 32
#define ARAW_FCNMM_WARPS_PER_BLOCK 8
#define ARAW_FCNMM_BLOCK_THREADS (ARAW_FCNMM_WARPS_PER_BLOCK * ARAW_FCNMM_WARP_THREADS)
...
// -----------------------------------------------------------------------
// ARAW_FCNMM: Aggregate-read / aggregate-write kernel
//   - One warp handles one stored row and one batch row.
//   - Reads batch-major matrix[n_batch, n_pre], scatters active rows to
//     output physically laid out as [n_batch, n_post].
// -----------------------------------------------------------------------
#define DEFINE_ARAW_FCNMM_HOMO(SUFFIX, SPIKE_T, IS_ACTIVE, WEIGHT_T, ACC_T, READ_W, ATOMIC_ADD_W) \
__global__ void _araw_fcnmm_homo_kern##SUFFIX(                                                    \
    const int32_t* __restrict__ indices,                                                          \
    const SPIKE_T* __restrict__ matrix,                                                           \
    WEIGHT_T*      __restrict__ output,                                                           \
    const WEIGHT_T* __restrict__ weights,                                                         \
    ...
    int warp_id = threadIdx.x >> 5;                                                                \
    int lane = threadIdx.x & 31;                                                                   \
    int row = static_cast<int>(blockIdx.x) * ARAW_FCNMM_WARPS_PER_BLOCK + warp_id;                 \
    int j = static_cast<int>(blockIdx.y);                                                          \
    if (row >= n_pre || j >= n_batch) return;                                                      \
    ACC_T w0 = READ_W(__ldg(&weights[0]));                                                         \
    if (!IS_ACTIVE(__ldg(&matrix[static_cast<size_t>(j) * n_pre + row]))) return;                  \
    const int32_t* i_row = indices + static_cast<size_t>(row) * n_conn;                            \
    for (int k = lane; k < n_conn; k += ARAW_FCNMM_WARP_THREADS) {                                 \
        int target = __ldg(&i_row[k]);                                                             \
        ATOMIC_ADD_W(&output[static_cast<size_t>(j) * n_post + target], w0);                       \
    }                                                                                              \
}

#define DEFINE_ARAW_FCNMM_HETERO(SUFFIX, SPIKE_T, IS_ACTIVE, WEIGHT_T, ACC_T, READ_W, ATOMIC_ADD_W) \
__global__ void _araw_fcnmm_hetero_kern##SUFFIX(                                                   \
    const int32_t* __restrict__ indices,                                                           \
    const SPIKE_T* __restrict__ matrix,                                                            \
    WEIGHT_T*      __restrict__ output,                                                            \
    const WEIGHT_T* __restrict__ weights,                                                          \
...
// ---- ARAW scatter kernel instantiations ----
DEFINE_ARAW_FCNMM_HOMO  (_bool_f32, uint8_t, IS_ACTIVE_BOOL, float, float, READ_F32, atomicAdd)
DEFINE_ARAW_FCNMM_HOMO  (_float_f32, float, IS_ACTIVE_F32, float, float, READ_F32, atomicAdd)
DEFINE_ARAW_FCNMM_HETERO(_bool_f32, uint8_t, IS_ACTIVE_BOOL, float, float, READ_F32, atomicAdd)
DEFINE_ARAW_FCNMM_HETERO(_float_f32, float, IS_ACTIVE_F32, float, float, READ_F32, atomicAdd)
DEFINE_ARAW_FCNMM_HOMO  (_bool_f64, uint8_t, IS_ACTIVE_BOOL, double, double, READ_F64, atomic_add_f64)
DEFINE_ARAW_FCNMM_HOMO  (_float_f64, double, IS_ACTIVE_F64, double, double, READ_F64, atomic_add_f64)
DEFINE_ARAW_FCNMM_HETERO(_bool_f64, uint8_t, IS_ACTIVE_BOOL, double, double, READ_F64, atomic_add_f64)
DEFINE_ARAW_FCNMM_HETERO(_float_f64, double, IS_ACTIVE_F64, double, double, READ_F64, atomic_add_f64)
DEFINE_ARAW_FCNMM_HOMO  (_bool_f16, uint8_t, IS_ACTIVE_BOOL, __half, float, READ_F16, atomic_add_f16)
DEFINE_ARAW_FCNMM_HOMO  (_float_f16, __half, IS_ACTIVE_F16, __half, float, READ_F16, atomic_add_f16)
DEFINE_ARAW_FCNMM_HETERO(_bool_f16, uint8_t, IS_ACTIVE_BOOL, __half, float, READ_F16, atomic_add_f16)
DEFINE_ARAW_FCNMM_HETERO(_float_f16, __half, IS_ACTIVE_F16, __half, float, READ_F16, atomic_add_f16)
DEFINE_ARAW_FCNMM_HOMO  (_bool_bf16, uint8_t, IS_ACTIVE_BOOL, __nv_bfloat16, float, READ_BF16, atomic_add_bf16)
DEFINE_ARAW_FCNMM_HOMO  (_float_bf16, __nv_bfloat16, IS_ACTIVE_BF16, __nv_bfloat16, float, READ_BF16, atomic_add_bf16)
DEFINE_ARAW_FCNMM_HETERO(_bool_bf16, uint8_t, IS_ACTIVE_BOOL, __nv_bfloat16, float, READ_BF16, atomic_add_bf16)
DEFINE_ARAW_FCNMM_HETERO(_float_bf16, __nv_bfloat16, IS_ACTIVE_BF16, __nv_bfloat16, float, READ_BF16, atomic_add_bf16)
...
// ---- FFI macro: ARAW scatter homo ----
#define FFI_ARAW_FCNMM_HOMO(SUFFIX, WEIGHT_C_T, SPIKE_C_T)                                                   \
void binary_fcnmm_araw_homo##SUFFIX(                                                                         \
    const BE::Tensor weights, const BE::Tensor indices,                                                       \
    const BE::Tensor matrix, BE::Tensor output, int64_t stream                                                \
) {                                                                                                           \
    cudaStream_t s = reinterpret_cast<cudaStream_t>(stream);                                                  \
    ...
    BE_CUDA_CHECK(cudaMemsetAsync(                                                                            \
        d_out, 0, static_cast<size_t>(output.size(0)) * output.size(1) * sizeof(WEIGHT_C_T), s));             \
    if (n_pre == 0 || n_conn == 0 || n_batch == 0) return;                                                    \
    int grid_x = (n_pre + ARAW_FCNMM_WARPS_PER_BLOCK - 1) / ARAW_FCNMM_WARPS_PER_BLOCK;                       \
    dim3 grid(grid_x, n_batch);                                                                               \
    _araw_fcnmm_homo_kern##SUFFIX<<<grid, ARAW_FCNMM_BLOCK_THREADS, 0, s>>>(                                   \
        d_idx, d_mat, d_out, d_w, n_pre, n_conn, n_post, n_batch);                                            \
    BE_CHECK_KERNEL_LAUNCH();                                                                                 \
}

// ---- FFI macro: ARAW scatter hetero ----
#define FFI_ARAW_FCNMM_HETERO(SUFFIX, WEIGHT_C_T, SPIKE_C_T)                                                 \
void binary_fcnmm_araw_hetero##SUFFIX(                                                                       \
    const BE::Tensor weights, const BE::Tensor indices,                                                       \
    const BE::Tensor matrix, BE::Tensor output, int64_t stream                                                \
) {                                                                                                           \
    cudaStream_t s = reinterpret_cast<cudaStream_t>(stream);                                                  \
    ...
    int grid_x = (n_pre + ARAW_FCNMM_WARPS_PER_BLOCK - 1) / ARAW_FCNMM_WARPS_PER_BLOCK;                       \
    dim3 grid(grid_x, n_batch);                                                                               \
    _araw_fcnmm_hetero_kern##SUFFIX<<<grid, ARAW_FCNMM_BLOCK_THREADS, 0, s>>>(                                 \
        d_idx, d_mat, d_out, d_w, n_pre, n_conn, n_post, n_batch);                                            \
    BE_CHECK_KERNEL_LAUNCH();                                                                                 \
}
...
// ---- ARAW scatter FFI ----
// @BE binary_fcnmm_araw_homo_bool_f32
FFI_ARAW_FCNMM_HOMO(_bool_f32, float, uint8_t)
// @BE binary_fcnmm_araw_homo_float_f32
FFI_ARAW_FCNMM_HOMO(_float_f32, float, float)
// @BE binary_fcnmm_araw_homo_bool_f64
FFI_ARAW_FCNMM_HOMO(_bool_f64, double, uint8_t)
// @BE binary_fcnmm_araw_homo_float_f64
FFI_ARAW_FCNMM_HOMO(_float_f64, double, double)
// @BE binary_fcnmm_araw_homo_bool_f16
FFI_ARAW_FCNMM_HOMO(_bool_f16, __half, uint8_t)
// @BE binary_fcnmm_araw_homo_float_f16
FFI_ARAW_FCNMM_HOMO(_float_f16, __half, __half)
// @BE binary_fcnmm_araw_homo_bool_bf16
FFI_ARAW_FCNMM_HOMO(_bool_bf16, __nv_bfloat16, uint8_t)
// @BE binary_fcnmm_araw_homo_float_bf16
FFI_ARAW_FCNMM_HOMO(_float_bf16, __nv_bfloat16, __nv_bfloat16)
// @BE binary_fcnmm_araw_hetero_bool_f32
FFI_ARAW_FCNMM_HETERO(_bool_f32, float, uint8_t)
// @BE binary_fcnmm_araw_hetero_float_f32
FFI_ARAW_FCNMM_HETERO(_float_f32, float, float)
// @BE binary_fcnmm_araw_hetero_bool_f64
FFI_ARAW_FCNMM_HETERO(_bool_f64, double, uint8_t)
// @BE binary_fcnmm_araw_hetero_float_f64
FFI_ARAW_FCNMM_HETERO(_float_f64, double, double)
// @BE binary_fcnmm_araw_hetero_bool_f16
FFI_ARAW_FCNMM_HETERO(_bool_f16, __half, uint8_t)
// @BE binary_fcnmm_araw_hetero_float_f16
FFI_ARAW_FCNMM_HETERO(_float_f16, __half, __half)
// @BE binary_fcnmm_araw_hetero_bool_bf16
FFI_ARAW_FCNMM_HETERO(_bool_bf16, __nv_bfloat16, uint8_t)
// @BE binary_fcnmm_araw_hetero_float_bf16
FFI_ARAW_FCNMM_HETERO(_float_bf16, __nv_bfloat16, __nv_bfloat16)

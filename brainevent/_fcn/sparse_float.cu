...
#define DEFINE_SPFLOAT_SCATTER_BASIC(SUFFIX, WEIGHT_T, ACC_T, READ_W, WRITE_W, ACC_ZERO) \
__global__ void _spfloat_scatter_basic_kern##SUFFIX( \
    ...
    for (int k = threadIdx.x; k < n_conn; k += blockDim.x) { \
        ...
        atomicAdd(&output[i_row[k]], WRITE_W(w_sp)); \
    } \
}

#define DEFINE_SPFLOAT_SCATTER_WARP(SUFFIX, WEIGHT_T, ACC_T, READ_W, WRITE_W, ACC_ZERO) \
...
DEFINE_SPFLOAT_SCATTER_BASIC(_f32, float, float, READ_F32, WRITE_F32, 0.0f)
DEFINE_SPFLOAT_SCATTER_WARP(_f32, float, float, READ_F32, WRITE_F32, 0.0f)
...
DEFINE_SPFLOAT_SCATTER_BASIC(_f64, double, double, READ_F64, WRITE_F64, 0.0)
DEFINE_SPFLOAT_SCATTER_WARP(_f64, double, double, READ_F64, WRITE_F64, 0.0)
...
DEFINE_SPFLOAT_SCATTER_BASIC(_f16, __half, float, READ_F16, WRITE_F16, 0.0f)
DEFINE_SPFLOAT_SCATTER_WARP(_f16, __half, float, READ_F16, WRITE_F16, 0.0f)
...
DEFINE_SPFLOAT_SCATTER_BASIC(_bf16, __nv_bfloat16, float, READ_BF16, WRITE_BF16, 0.0f)
DEFINE_SPFLOAT_SCATTER_WARP(_bf16, __nv_bfloat16, float, READ_BF16, WRITE_BF16, 0.0f)

// SpMV FFI Entry Macros
#define FFI_SPFLOAT_GATHER_AUTO(SUFFIX, WEIGHT_C_T, SHM_SIZE) \
void spfloat_fcnmv_gather_auto##SUFFIX(tvm::ffi::TensorView weights, tvm::ffi::TensorView indices, tvm::ffi::TensorView vector, tvm::ffi::TensorView output, int64_t stream) { \
    ...
    if (n_conn <= 64) _spfloat_gather_warp_kern##SUFFIX<<<n_pre, 32, 0, s>>>(d_idx, d_vec, d_out, d_w, n_pre, n_conn, is_homo); \
    else _spfloat_gather_basic_kern##SUFFIX<<<n_pre, 256, SHM_SIZE, s>>>(d_idx, d_vec, d_out, d_w, n_pre, n_conn, is_homo); \
}
#define FFI_SPFLOAT_SCATTER_AUTO(SUFFIX, WEIGHT_C_T) \
void spfloat_fcnmv_scatter_auto##SUFFIX(tvm::ffi::TensorView weights, tvm::ffi::TensorView indices, tvm::ffi::TensorView vector, tvm::ffi::TensorView output, int64_t stream) { \
    ...
    if (n_conn <= 32) { int blocks = (n_pre + 7) / 8; _spfloat_scatter_warp_kern##SUFFIX<<<blocks, 256, 0, s>>>(d_idx, d_vec, d_out, d_w, n_pre, n_conn, is_homo); } \
    else _spfloat_scatter_basic_kern##SUFFIX<<<n_pre, 256, 0, s>>>(d_idx, d_vec, d_out, d_w, n_pre, n_conn, is_homo); \
}

// SpMV FFI Instantiations
// @tvm_ffi spfloat_fcnmv_gather_warp_f32
void spfloat_fcnmv_gather_warp_f32(tvm::ffi::TensorView weights, tvm::ffi::TensorView indices, tvm::ffi::TensorView vector, tvm::ffi::TensorView output, int64_t stream) {
    cudaStream_t s = (cudaStream_t)stream; int n_pre = indices.size(0), n_conn = indices.size(1), is_homo = (weights.ndim() == 1) ? 1 : 0;
    _spfloat_gather_warp_kern_f32<<<n_pre, 32, 0, s>>>((const int32_t*)indices.data_ptr(), (const float*)vector.data_ptr(), (float*)output.data_ptr(), (const float*)weights.data_ptr(), n_pre, n_conn, is_homo);
}
// @tvm_ffi spfloat_fcnmv_gather_basic_f32
void spfloat_fcnmv_gather_basic_f32(tvm::ffi::TensorView weights, tvm::ffi::TensorView indices, tvm::ffi::TensorView vector, tvm::ffi::TensorView output, int64_t stream) {
    cudaStream_t s = (cudaStream_t)stream; int n_pre = indices.size(0), n_conn = indices.size(1), is_homo = (weights.ndim() == 1) ? 1 : 0;
    _spfloat_gather_basic_kern_f32<<<n_pre, 256, 32 * sizeof(float), s>>>((const int32_t*)indices.data_ptr(), (const float*)vector.data_ptr(), (float*)output.data_ptr(), (const float*)weights.data_ptr(), n_pre, n_conn, is_homo);
}
// @tvm_ffi spfloat_fcnmv_scatter_warp_f32
void spfloat_fcnmv_scatter_warp_f32(tvm::ffi::TensorView weights, tvm::ffi::TensorView indices, tvm::ffi::TensorView vector, tvm::ffi::TensorView output, int64_t stream) {
    cudaStream_t s = (cudaStream_t)stream; int n_pre = indices.size(0), n_conn = indices.size(1), n_post = output.size(0), is_homo = (weights.ndim() == 1) ? 1 : 0;
    cudaMemsetAsync(output.data_ptr(), 0, (size_t)n_post * sizeof(float), s);
    int blocks = (n_pre + 7) / 8; _spfloat_scatter_warp_kern_f32<<<blocks, 256, 0, s>>>((const int32_t*)indices.data_ptr(), (const float*)vector.data_ptr(), (float*)output.data_ptr(), (const float*)weights.data_ptr(), n_pre, n_conn, is_homo);
}
// @tvm_ffi spfloat_fcnmv_scatter_auto_f32
FFI_SPFLOAT_SCATTER_AUTO(_f32, float)
// @tvm_ffi spfloat_fcnmv_gather_warp_f64
void spfloat_fcnmv_gather_warp_f64(tvm::ffi::TensorView weights, tvm::ffi::TensorView indices, tvm::ffi::TensorView vector, tvm::ffi::TensorView output, int64_t stream) {
    cudaStream_t s = (cudaStream_t)stream; int n_pre = indices.size(0), n_conn = indices.size(1), is_homo = (weights.ndim() == 1) ? 1 : 0;
    _spfloat_gather_warp_kern_f64<<<n_pre, 32, 0, s>>>((const int32_t*)indices.data_ptr(), (const double*)vector.data_ptr(), (double*)output.data_ptr(), (const double*)weights.data_ptr(), n_pre, n_conn, is_homo);
}
// @tvm_ffi spfloat_fcnmv_gather_basic_f64
void spfloat_fcnmv_gather_basic_f64(tvm::ffi::TensorView weights, tvm::ffi::TensorView indices, tvm::ffi::TensorView vector, tvm::ffi::TensorView output, int64_t stream) {
    cudaStream_t s = (cudaStream_t)stream; int n_pre = indices.size(0), n_conn = indices.size(1), is_homo = (weights.ndim() == 1) ? 1 : 0;
    _spfloat_gather_basic_kern_f64<<<n_pre, 256, 32 * sizeof(double), s>>>((const int32_t*)indices.data_ptr(), (const double*)vector.data_ptr(), (double*)output.data_ptr(), (const double*)weights.data_ptr(), n_pre, n_conn, is_homo);
}
// @tvm_ffi spfloat_fcnmv_scatter_auto_f64
FFI_SPFLOAT_SCATTER_AUTO(_f64, double)
// @tvm_ffi spfloat_fcnmv_gather_warp_f16
void spfloat_fcnmv_gather_warp_f16(tvm::ffi::TensorView weights, tvm::ffi::TensorView indices, tvm::ffi::TensorView vector, tvm::ffi::TensorView output, int64_t stream) {
    cudaStream_t s = (cudaStream_t)stream; int n_pre = indices.size(0), n_conn = indices.size(1), is_homo = (weights.ndim() == 1) ? 1 : 0;
    _spfloat_gather_warp_kern_f16<<<n_pre, 32, 0, s>>>((const int32_t*)indices.data_ptr(), (const __half*)vector.data_ptr(), (__half*)output.data_ptr(), (const __half*)weights.data_ptr(), n_pre, n_conn, is_homo);
}
// @tvm_ffi spfloat_fcnmv_gather_basic_f16
void spfloat_fcnmv_gather_basic_f16(tvm::ffi::TensorView weights, tvm::ffi::TensorView indices, tvm::ffi::TensorView vector, tvm::ffi::TensorView output, int64_t stream) {
    cudaStream_t s = (cudaStream_t)stream; int n_pre = indices.size(0), n_conn = indices.size(1), is_homo = (weights.ndim() == 1) ? 1 : 0;
    _spfloat_gather_basic_kern_f16<<<n_pre, 256, 32 * sizeof(float), s>>>((const int32_t*)indices.data_ptr(), (const __half*)vector.data_ptr(), (__half*)output.data_ptr(), (const __half*)weights.data_ptr(), n_pre, n_conn, is_homo);
}
...
void spfloat_densemm_nt##SUFFIX(tvm::ffi::TensorView weights, tvm::ffi::TensorView spikes, tvm::ffi::TensorView output, int64_t stream) { \
    cudaStream_t s = (cudaStream_t)stream; int m = weights.size(0), k = weights.size(1), n = spikes.size(1), warps = 8, m_blks = (m + warps - 1) / warps, n_chnks = (n + CHUNK_N_VAL - 1) / CHUNK_N_VAL; \
    dim3 grid(m_blks, n_chnks); _spfloat_mm_nt_wpr_kern##SUFFIX<<<grid, 256, 0, s>>>((const WEIGHT_C_T*)weights.data_ptr(), (const WEIGHT_C_T*)spikes.data_ptr(), (WEIGHT_C_T*)output.data_ptr(), m, k, n); \
}
#define FFI_SPFLOAT_MM_NT_TPE(SUFFIX, WEIGHT_C_T) \
void spfloat_densemm_nt_tpe##SUFFIX(tvm::ffi::TensorView weights, tvm::ffi::TensorView spikes, tvm::ffi::TensorView output, int64_t stream) { \
    cudaStream_t s = (cudaStream_t)stream; int m = weights.size(0), k = weights.size(1), n = spikes.size(1), warps = 8, m_blks = (m + warps - 1) / warps, n_blks = (n + 31) / 32; \
    dim3 grid(m_blks, n_blks); _spfloat_mm_nt_tpe_kern##SUFFIX<<<grid, 256, 0, s>>>((const WEIGHT_C_T*)weights.data_ptr(), (const WEIGHT_C_T*)spikes.data_ptr(), (WEIGHT_C_T*)output.data_ptr(), m, k, n); \
}
#define FFI_SPFLOAT_MM_T(SUFFIX, WEIGHT_C_T, CHUNK_N_VAL) \
void spfloat_densemm_t##SUFFIX(tvm::ffi::TensorView weights, tvm::ffi::TensorView spikes, tvm::ffi::TensorView output, int64_t stream) { \
    cudaStream_t s = (cudaStream_t)stream; int k = weights.size(0), n = weights.size(1), m = spikes.size(0), warps = 8, m_blks = (m + warps - 1) / warps, n_chnks = (n + CHUNK_N_VAL - 1) / CHUNK_N_VAL; \
    dim3 grid(m_blks, n_chnks); _spfloat_mm_t_kern##SUFFIX<<<grid, 256, 0, s>>>((const WEIGHT_C_T*)weights.data_ptr(), (const WEIGHT_C_T*)spikes.data_ptr(), (WEIGHT_C_T*)output.data_ptr(), m, k, n); \
}

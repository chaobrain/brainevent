...
from brainevent import fcnmv_dt2t, fcnmm_dt2t  # top-level exports
from brainevent._fcn.dt2t import fcnmv_dt2t as fcnmv_dt2t_module  # module-level export
from brainevent._fcn.dt2t import fcnmm_dt2t as fcnmm_dt2t_module  # module-level export
...
# ============================================================================ #
# fcnmm_dt2t: batched variant — ``weights`` (n_batch, rows, n_conn) and
# ``y`` (n_batch, n_hidden) share a leading batch axis (D-RTRL batches).
# ============================================================================ #

n_batch = 3


def _reference_mm(w, indices, y, transpose):
    """Hand-rolled reference: out[b, i, k] = w[b, i, k] * y[b, row/col]."""
    w_full = jnp.broadcast_to(w, (y.shape[0], *indices.shape))
    yv = y[:, indices] if transpose else y[:, :, None]
    return w_full * yv


def test_fcnmm_module_and_toplevel_are_same_object():
    # The dedicated module is the single source of truth; the top-level export
    # must be the very same function object.
    assert fcnmm_dt2t is fcnmm_dt2t_module


@pytest.mark.parametrize('homo_w', [True, False])
@pytest.mark.parametrize('transpose', [True, False])
def test_fcnmm_dt2t(homo_w, transpose):
    m, n = shape  # (20, 40)
    indices = generate_fixed_conn_num_indices(m, n, n_conn, replace=True)
    if homo_w:
        w = jnp.asarray(1.5, dtype=jnp.float32)
    else:
        w = jnp.arange(1, n_batch * indices.size + 1, dtype=jnp.float32).reshape(n_batch, *indices.shape)
    y = jnp.arange(1, n_batch * (m if not transpose else n) + 1, dtype=jnp.float32)
    y = y.reshape(n_batch, (m if not transpose else n))

    out = fcnmm_dt2t(w, indices, y, shape=shape, transpose=transpose)

    assert out.shape == (n_batch, *indices.shape)
    expected = _reference_mm(w, indices, y, transpose)
    assert allclose(out, expected, rtol=1e-5, atol=1e-5)


@pytest.mark.parametrize('transpose', [True, False])
def test_fcnmm_dt2t_matches_stacked_mv(transpose):
    ...
    mm_out = fcnmm_dt2t(w, indices, y, shape=shape, transpose=transpose)
    mv_out = jnp.stack([
        fcnmv_dt2t(w[b], indices, y[b], shape=shape, transpose=transpose)
        for b in range(n_batch)
    ])
    assert allclose(mm_out, mv_out, rtol=1e-5, atol=1e-5)


def test_fcnmm_dt2t_homo_size1_array_equiv_scalar():
    # A size-1 array weight must behave identically to a 0-d scalar weight.
    m, n = shape
    indices = generate_fixed_conn_num_indices(m, n, n_conn, replace=True)
    y = jnp.arange(1, n_batch * m + 1, dtype=jnp.float32).reshape(n_batch, m)
    scalar = jnp.asarray(1.5, dtype=jnp.float32)
    size1 = jnp.asarray([1.5], dtype=jnp.float32)
    out_scalar = fcnmm_dt2t(scalar, indices, y, shape=shape, transpose=False)
    out_size1 = fcnmm_dt2t(size1, indices, y, shape=shape, transpose=False)
    assert out_scalar.shape == out_size1.shape == (n_batch, *indices.shape)
    assert allclose(out_scalar, out_size1, rtol=1e-6, atol=1e-6)


def _setup_mm_unit_case():
    m, n = shape
    indices = generate_fixed_conn_num_indices(m, n, n_conn, replace=True)
    w_mant = jnp.arange(1, n_batch * indices.size + 1, dtype=jnp.float32).reshape(n_batch, *indices.shape)
    y_mant = jnp.arange(1, n_batch * m + 1, dtype=jnp.float32).reshape(n_batch, m)
    return indices, w_mant, y_mant


def test_fcnmm_dt2t_units_both():
    indices, w_mant, y_mant = _setup_mm_unit_case()
    out = fcnmm_dt2t(w_mant * u.siemens, indices, y_mant * u.mV, shape=shape, transpose=False)
    expected = (w_mant * y_mant[:, :, None]) * (u.siemens * u.mV)
    assert u.math.allclose(out, expected)


def test_fcnmm_dt2t_units_weight_only():
    indices, w_mant, y_mant = _setup_mm_unit_case()
    out = fcnmm_dt2t(w_mant * u.siemens, indices, y_mant, shape=shape, transpose=False)
    expected = (w_mant * y_mant[:, :, None]) * u.siemens
    assert u.math.allclose(out, expected)


def test_fcnmm_dt2t_units_y_only():
    indices, w_mant, y_mant = _setup_mm_unit_case()
    out = fcnmm_dt2t(w_mant, indices, y_mant * u.mV, shape=shape, transpose=False)
    expected = (w_mant * y_mant[:, :, None]) * u.mV
    assert u.math.allclose(out, expected)


def test_fcnmm_dt2t_units_none_is_plain_array():
    indices, w_mant, y_mant = _setup_mm_unit_case()
    out = fcnmm_dt2t(w_mant, indices, y_mant, shape=shape, transpose=False)
    assert not isinstance(out, u.Quantity)
    assert allclose(out, w_mant * y_mant[:, :, None], rtol=1e-5, atol=1e-5)
...
@pytest.mark.parametrize('homo_w', [True, False])
@pytest.mark.parametrize('transpose', [True, False])
def test_fcnmm_dt2t_grad(homo_w, transpose):
    m, n = shape
    indices = generate_fixed_conn_num_indices(m, n, n_conn, replace=True)
    if homo_w:
        w = jnp.asarray(1.5, dtype=jnp.float32)
    ...
    def loss(w_, y_):
        return jnp.sum(fcnmm_dt2t(w_, indices, y_, shape=shape, transpose=transpose) ** 2)

    def ref(w_, y_):
        return jnp.sum(_reference_mm(w_, indices, y_, transpose) ** 2)
...
@pytest.mark.parametrize('transpose', [True, False])
def test_fcnmm_dt2t_jvp(transpose):
    m, n = shape
    indices = generate_fixed_conn_num_indices(m, n, n_conn, replace=True)
    w = jnp.arange(1, n_batch * indices.size + 1, dtype=jnp.float32).reshape(n_batch, *indices.shape)
    y = jnp.arange(1, n_batch * (m if not transpose else n) + 1, dtype=jnp.float32)
    ...
    f = lambda w_, y_: fcnmm_dt2t(w_, indices, y_, shape=shape, transpose=transpose)
    out, out_dot = jax.jvp(f, (w, y), (dw, dy))
    ...
    dyv = dy[:, indices] if transpose else dy[:, :, None]
    expected_dot = dw * yv + w * dyv
    assert allclose(out, _reference_mm(w, indices, y, transpose), rtol=1e-5, atol=1e-5)
    assert allclose(out_dot, expected_dot, rtol=1e-4, atol=1e-4)


def test_fcnmm_dt2t_jit():
    m, n = shape
    indices = generate_fixed_conn_num_indices(m, n, n_conn, replace=True)
    w = jnp.arange(1, n_batch * indices.size + 1, dtype=jnp.float32).reshape(n_batch, *indices.shape)
    y = jnp.arange(1, n_batch * m + 1, dtype=jnp.float32).reshape(n_batch, m)

    fn = jax.jit(lambda w_, y_: fcnmm_dt2t(w_, indices, y_, shape=shape, transpose=False))
    out = fn(w, y)
    assert allclose(out, w * y[:, :, None], rtol=1e-5, atol=1e-5)


def test_fcnmm_dt2t_vmap():
    # An outer vmap over an extra leading axis of both operands.
    m, n = shape
    indices = generate_fixed_conn_num_indices(m, n, n_conn, replace=True)
    W = jnp.arange(1, 2 * n_batch * indices.size + 1, dtype=jnp.float32).reshape(2, n_batch, *indices.shape)
    Y = jnp.arange(1, 2 * n_batch * m + 1, dtype=jnp.float32).reshape(2, n_batch, m)

    out = jax.vmap(lambda w_, y_: fcnmm_dt2t(w_, indices, y_, shape=shape, transpose=False))(W, Y)
    assert out.shape == (2, n_batch, *indices.shape)
    expected = W * Y[:, :, :, None]
    assert allclose(out, expected, rtol=1e-5, atol=1e-5)
...
@pytest.mark.parametrize('transpose', [True, False])
@pytest.mark.parametrize('homo_w', [True, False])
def test_fcnmm_dt2t_empty_rows(homo_w, transpose):
    # rows == 0: indices (0, n_conn).
    n = 40
    indices = jnp.zeros((0, n_conn), dtype=jnp.int32)
    shp = (0, n)
    w = (jnp.asarray(1.5, dtype=jnp.float32) if homo_w
         else jnp.zeros((n_batch, 0, n_conn), dtype=jnp.float32))
    y = jnp.zeros((n_batch, n if transpose else 0), dtype=jnp.float32)
    out = fcnmm_dt2t(w, indices, y, shape=shp, transpose=transpose)
    assert out.shape == (n_batch, 0, n_conn)


@pytest.mark.parametrize('transpose', [True, False])
@pytest.mark.parametrize('homo_w', [True, False])
def test_fcnmm_dt2t_empty_conn(homo_w, transpose):
    # n_conn == 0: indices (rows, 0).
    m, n = shape
    indices = jnp.zeros((m, 0), dtype=jnp.int32)
    w = (jnp.asarray(1.5, dtype=jnp.float32) if homo_w
         else jnp.zeros((n_batch, m, 0), dtype=jnp.float32))
    y = jnp.arange(1, n_batch * (m if not transpose else n) + 1, dtype=jnp.float32)
    y = y.reshape(n_batch, (m if not transpose else n))
    out = fcnmm_dt2t(w, indices, y, shape=shape, transpose=transpose)
    assert out.shape == (n_batch, m, 0)


def test_fcnmm_dt2t_zero_batch():
    # n_batch == 0: y (0, m) -> out (0, rows, n_conn).
    m, n = shape
    indices = generate_fixed_conn_num_indices(m, n, n_conn, replace=True)
    w = jnp.zeros((0, *indices.shape), dtype=jnp.float32)
    y = jnp.zeros((0, m), dtype=jnp.float32)
    out = fcnmm_dt2t(w, indices, y, shape=shape, transpose=False)
    assert out.shape == (0, *indices.shape)


def test_fcnmm_dt2t_validation():
    m, n = shape
    indices = generate_fixed_conn_num_indices(m, n, n_conn, replace=True)
    w = jnp.ones((n_batch, *indices.shape), dtype=jnp.float32)

    # indices not 2D
    with pytest.raises(ValueError):
        fcnmm_dt2t(jnp.ones((n_batch, n_conn), dtype=jnp.float32), jnp.arange(n_conn, dtype=jnp.int32),
                   jnp.ones((n_batch, m), dtype=jnp.float32), shape=shape, transpose=False)
    # shape not length-2
    with pytest.raises(ValueError):
        fcnmm_dt2t(w, indices, jnp.ones((n_batch, m), dtype=jnp.float32), shape=(m, n, 1), transpose=False)
    # non-floating weights
    with pytest.raises(ValueError):
        fcnmm_dt2t(jnp.ones((n_batch, *indices.shape), dtype=jnp.int32), indices,
                   jnp.ones((n_batch, m), dtype=jnp.float32), shape=shape, transpose=False)
    # weight shape mismatch (and not size-1)
    with pytest.raises(ValueError):
        fcnmm_dt2t(jnp.ones((n_batch, m, n_conn + 1), dtype=jnp.float32), indices,
                   jnp.ones((n_batch, m), dtype=jnp.float32), shape=shape, transpose=False)
    # weight batch mismatch with y batch
    with pytest.raises(ValueError):
        fcnmm_dt2t(jnp.ones((n_batch + 1, *indices.shape), dtype=jnp.float32), indices,
                   jnp.ones((n_batch, m), dtype=jnp.float32), shape=shape, transpose=False)
    # unbatched (2D) heterogeneous weights are rejected for the mm variant
    with pytest.raises(ValueError):
        fcnmm_dt2t(jnp.ones(indices.shape, dtype=jnp.float32), indices,
                   jnp.ones((n_batch, m), dtype=jnp.float32), shape=shape, transpose=False)
    # y not 2D
    with pytest.raises(ValueError):
        fcnmm_dt2t(w, indices, jnp.ones(m, dtype=jnp.float32), shape=shape, transpose=False)
    # wrong y trailing dim for transpose=False (expects shape[0]=m)
    with pytest.raises(ValueError):
        fcnmm_dt2t(w, indices, jnp.ones((n_batch, m + 3), dtype=jnp.float32), shape=shape, transpose=False)
    # wrong y trailing dim for transpose=True (expects shape[1]=n)
    with pytest.raises(ValueError):
        fcnmm_dt2t(w, indices, jnp.ones((n_batch, n + 3), dtype=jnp.float32), shape=shape, transpose=True)

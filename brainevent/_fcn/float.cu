...
void fcnmv_gather_auto##SUFFIX(tvm::ffi::TensorView weights, tvm::ffi::TensorView indices, tvm::ffi::TensorView vector, tvm::ffi::TensorView output, int64_t stream) { \
    cudaStream_t s = (cudaStream_t)stream; int n_pre = indices.size(0), n_conn = indices.size(1), is_homo = (weights.ndim() == 1) ? 1 : 0; \
    const WEIGHT_C_T *d_w = (const WEIGHT_C_T*)weights.data_ptr(), *d_vec = (const WEIGHT_C_T*)vector.data_ptr(); \
    const int32_t *d_idx = (const int32_t*)indices.data_ptr(); WEIGHT_C_T *d_out = (WEIGHT_C_T*)output.data_ptr(); \
    if (n_conn <= 32) _gather_warp_kern##SUFFIX<<<n_pre, 32, 0, s>>>(d_idx, d_vec, d_out, d_w, n_pre, n_conn, is_homo); \
    else _gather_basic_kern##SUFFIX<<<n_pre, 256, SHM_SIZE, s>>>(d_idx, d_vec, d_out, d_w, n_pre, n_conn, is_homo); \
}
#define FFI_SCATTER_AUTO(SUFFIX, WEIGHT_C_T) \
void fcnmv_scatter_auto##SUFFIX(tvm::ffi::TensorView weights, tvm::ffi::TensorView indices, tvm::ffi::TensorView vector, tvm::ffi::TensorView output, int64_t stream) { \
    cudaStream_t s = (cudaStream_t)stream; int n_pre = indices.size(0), n_conn = indices.size(1), n_post = output.size(0), is_homo = (weights.ndim() == 1) ? 1 : 0; \
    const WEIGHT_C_T *d_w = (const WEIGHT_C_T*)weights.data_ptr(), *d_vec = (const WEIGHT_C_T*)vector.data_ptr(); \
    const int32_t *d_idx = (const int32_t*)indices.data_ptr(); WEIGHT_C_T *d_out = (WEIGHT_C_T*)output.data_ptr(); \
    cudaMemsetAsync(d_out, 0, (size_t)n_post * sizeof(WEIGHT_C_T), s); \
    if (n_conn <= 32) { int blocks = (n_pre + 7) / 8; _scatter_warp_kern##SUFFIX<<<blocks, 256, 0, s>>>(d_idx, d_vec, d_out, d_w, n_pre, n_conn, is_homo); } \
    else if ((long long)n_pre * n_conn > 262144LL) { int blocks = min(1024, (int)((n_pre * n_conn + 255) / 256)); _scatter_gs_kern##SUFFIX<<<blocks, 256, 0, s>>>(d_idx, d_vec, d_out, d_w, n_pre, n_conn, is_homo); } \
    else { _scatter_basic_kern##SUFFIX<<<n_pre, 256, 0, s>>>(d_idx, d_vec, d_out, d_w, n_pre, n_conn, is_homo); } \
}
// @tvm_ffi fcnmv_gather_warp_f32
void fcnmv_gather_warp_f32(tvm::ffi::TensorView weights, tvm::ffi::TensorView indices, tvm::ffi::TensorView vector, tvm::ffi::TensorView output, int64_t stream) {
    cudaStream_t s = (cudaStream_t)stream; int n_pre = indices.size(0), n_conn = indices.size(1), is_homo = (weights.ndim() == 1) ? 1 : 0;
    _gather_warp_kern_f32<<<n_pre, 32, 0, s>>>((const int32_t*)indices.data_ptr(), (const float*)vector.data_ptr(), (float*)output.data_ptr(), (const float*)weights.data_ptr(), n_pre, n_conn, is_homo);
}
// @tvm_ffi fcnmv_gather_basic_f32
void fcnmv_gather_basic_f32(tvm::ffi::TensorView weights, tvm::ffi::TensorView indices, tvm::ffi::TensorView vector, tvm::ffi::TensorView output, int64_t stream) {
    cudaStream_t s = (cudaStream_t)stream; int n_pre = indices.size(0), n_conn = indices.size(1), is_homo = (weights.ndim() == 1) ? 1 : 0;
    _gather_basic_kern_f32<<<n_pre, 256, 32 * sizeof(float), s>>>((const int32_t*)indices.data_ptr(), (const float*)vector.data_ptr(), (float*)output.data_ptr(), (const float*)weights.data_ptr(), n_pre, n_conn, is_homo);
}
// @tvm_ffi fcnmv_scatter_warp_f32
void fcnmv_scatter_warp_f32(tvm::ffi::TensorView weights, tvm::ffi::TensorView indices, tvm::ffi::TensorView vector, tvm::ffi::TensorView output, int64_t stream) {
    cudaStream_t s = (cudaStream_t)stream; int n_pre = indices.size(0), n_conn = indices.size(1), n_post = output.size(0), is_homo = (weights.ndim() == 1) ? 1 : 0;
    cudaMemsetAsync(output.data_ptr(), 0, (size_t)n_post * sizeof(float), s);
    int blocks = (n_pre + 7) / 8; _scatter_warp_kern_f32<<<blocks, 256, 0, s>>>((const int32_t*)indices.data_ptr(), (const float*)vector.data_ptr(), (float*)output.data_ptr(), (const float*)weights.data_ptr(), n_pre, n_conn, is_homo);
}
...
FFI_SCATTER_AUTO(_bf16, __nv_bfloat16)
void fcnmv_gather_vec4_f32(tvm::ffi::TensorView weights, tvm::ffi::TensorView indices, tvm::ffi::TensorView vector, tvm::ffi::TensorView output, int64_t stream) {
    cudaStream_t s = reinterpret_cast<cudaStream_t>(stream);
    int n_pre  = indices.size(0), n_conn = indices.size(1), is_homo = (weights.ndim() == 1) ? 1 : 0;
    _gather_vec4_kern<<<n_pre, 256, 32 * sizeof(float), s>>>((const int32_t*)indices.data_ptr(), (const float*)vector.data_ptr(), (float*)output.data_ptr(), (const float*)weights.data_ptr(), n_pre, n_conn, is_homo);
}
void fcnmv_gather_auto_f32(tvm::ffi::TensorView weights, tvm::ffi::TensorView indices, tvm::ffi::TensorView vector, tvm::ffi::TensorView output, int64_t stream) {
    cudaStream_t s = reinterpret_cast<cudaStream_t>(stream);
    int n_pre  = indices.size(0), n_conn = indices.size(1), is_homo = (weights.ndim() == 1) ? 1 : 0;
    if (n_conn <= 32) _gather_warp_kern_f32<<<n_pre, 32, 0, s>>>((const int32_t*)indices.data_ptr(), (const float*)vector.data_ptr(), (float*)output.data_ptr(), (const float*)weights.data_ptr(), n_pre, n_conn, is_homo);
    else if (n_conn % 4 == 0 && n_conn >= 128) _gather_vec4_kern<<<n_pre, 256, 32 * sizeof(float), s>>>((const int32_t*)indices.data_ptr(), (const float*)vector.data_ptr(), (float*)output.data_ptr(), (const float*)weights.data_ptr(), n_pre, n_conn, is_homo);
    else if (n_conn > 512) { int threads = 256; size_t shm = (size_t)threads * (sizeof(int32_t) + sizeof(float)) + 32 * sizeof(float); _gather_shared_kern<<<n_pre, threads, shm, s>>>((const int32_t*)indices.data_ptr(), (const float*)vector.data_ptr(), (float*)output.data_ptr(), (const float*)weights.data_ptr(), n_pre, n_conn, is_homo); }
    else _gather_basic_kern_f32<<<n_pre, 256, 32 * sizeof(float), s>>>((const int32_t*)indices.data_ptr(), (const float*)vector.data_ptr(), (float*)output.data_ptr(), (const float*)weights.data_ptr(), n_pre, n_conn, is_homo);
}
...
__global__ void _mm_scatter_block_kern##SUFFIX( \
    ...
    for (int k = 0; k < n_conn; k++) { \
        ...
        for (int j = threadIdx.x; j < n_col; j += blockDim.x) \
            atomicAdd(&out_row[j], READ_W(w) * READ_W(m_row[j])); \
    } \
}
...
__global__ void _mm_scatter_cached_kern(const int32_t* __restrict__ indices, const float* __restrict__ matrix, float* __restrict__ output, const float* __restrict__ weights, int n_pre, int n_conn, int n_col, int is_homo) {
    ...
    const int32_t* idx_row = indices + (size_t)i * n_conn; const float* w_row = is_homo ? nullptr : weights + (size_t)i * n_conn;
    if (j < n_col) { float m_val = s_m[threadIdx.x]; for (int k = 0; k < n_conn; k++) { int tgt = idx_row[k]; float w = is_homo ? weights[0] : w_row[k]; atomicAdd(&output[(size_t)tgt * n_col + j], w * m_val); } }
}

// SpMM FFI Entries
#define FFI_MM_GATHER_AUTO(SUFFIX, WEIGHT_C_T) \
void fcnmm_gather_auto##SUFFIX(tvm::ffi::TensorView weights, tvm::ffi::TensorView indices, tvm::ffi::TensorView matrix,  tvm::ffi::TensorView output, int64_t stream) { \
    cudaStream_t s = (cudaStream_t)stream; int n_pre  = indices.size(0), n_conn = indices.size(1), n_col = matrix.size(1), is_homo = (weights.ndim() == 1) ? 1 : 0; \
    _mm_gather_basic_kern##SUFFIX<<<dim3(n_pre, (n_col + 63) / 64), 64, 0, s>>>((const int32_t*)indices.data_ptr(), (const WEIGHT_C_T*)matrix.data_ptr(), (WEIGHT_C_T*)output.data_ptr(), (const WEIGHT_C_T*)weights.data_ptr(), n_pre, n_conn, n_col, is_homo); \
}
#define FFI_MM_SCATTER_AUTO(SUFFIX, WEIGHT_C_T) \
void fcnmm_scatter_auto##SUFFIX(tvm::ffi::TensorView weights, tvm::ffi::TensorView indices, tvm::ffi::TensorView matrix,  tvm::ffi::TensorView output, int64_t stream) { \
    cudaStream_t s = (cudaStream_t)stream; int n_pre  = indices.size(0), n_conn = indices.size(1), n_post = output.size(0), n_col = matrix.size(1), is_homo = (weights.ndim() == 1) ? 1 : 0; \
    cudaMemsetAsync(output.data_ptr(), 0, (size_t)n_post * n_col * sizeof(WEIGHT_C_T), s); \
    if (n_col <= 64) { int n_pairs = n_pre * n_conn; int blocks = min(4096, (n_pairs + 7) / 8); _mm_scatter_warp_kern##SUFFIX<<<blocks, 256, 0, s>>>((const int32_t*)indices.data_ptr(), (const WEIGHT_C_T*)matrix.data_ptr(), (WEIGHT_C_T*)output.data_ptr(), (const WEIGHT_C_T*)weights.data_ptr(), n_pre, n_conn, n_col, is_homo); } \
    else { _mm_scatter_block_kern##SUFFIX<<<n_pre, 256, 0, s>>>((const int32_t*)indices.data_ptr(), (const WEIGHT_C_T*)matrix.data_ptr(), (WEIGHT_C_T*)output.data_ptr(), (const WEIGHT_C_T*)weights.data_ptr(), n_pre, n_conn, n_col, is_homo); } \
}
...
void fcnmm_gather_auto_f32(tvm::ffi::TensorView weights, tvm::ffi::TensorView indices, tvm::ffi::TensorView matrix, tvm::ffi::TensorView output, int64_t stream) {
    cudaStream_t s = (cudaStream_t)stream; int n_pre = indices.size(0), n_conn = indices.size(1), n_col = matrix.size(1), is_homo = (weights.ndim() == 1) ? 1 : 0;
    if (n_col % 4 == 0 && n_col >= 64) { dim3 grid(n_pre, (n_col / 4 + 63) / 64); _mm_gather_vec4_kern<<<grid, 64, 0, s>>>((const int32_t*)indices.data_ptr(), (const float*)matrix.data_ptr(), (float*)output.data_ptr(), (const float*)weights.data_ptr(), n_pre, n_conn, n_col, is_homo); }
    else if (n_conn > 128) { dim3 grid(n_pre, (n_col + 63) / 64); size_t shm = MMTK * (sizeof(int32_t) + sizeof(float)); _mm_gather_shared_kern<<<grid, 64, shm, s>>>((const int32_t*)indices.data_ptr(), (const float*)matrix.data_ptr(), (float*)output.data_ptr(), (const float*)weights.data_ptr(), n_pre, n_conn, n_col, is_homo); }
    else { dim3 grid(n_pre, (n_col + 63) / 64); _mm_gather_basic_kern_f32<<<grid, 64, 0, s>>>((const int32_t*)indices.data_ptr(), (const float*)matrix.data_ptr(), (float*)output.data_ptr(), (const float*)weights.data_ptr(), n_pre, n_conn, n_col, is_homo); }
}
void fcnmm_scatter_auto_f32(tvm::ffi::TensorView weights, tvm::ffi::TensorView indices, tvm::ffi::TensorView matrix, tvm::ffi::TensorView output, int64_t stream) {
    cudaStream_t s = (cudaStream_t)stream; int n_pre = indices.size(0), n_conn = indices.size(1), n_post = output.size(0), n_col = matrix.size(1), is_homo = (weights.ndim() == 1) ? 1 : 0;
    cudaMemsetAsync(output.data_ptr(), 0, (size_t)n_post * n_col * sizeof(float), s);
    if (n_col <= 64) { int n_pairs = n_pre * n_conn; int blocks = min(4096, (n_pairs + 7) / 8); _mm_scatter_warp_kern_f32<<<blocks, 256, 0, s>>>((const int32_t*)indices.data_ptr(), (const float*)matrix.data_ptr(), (float*)output.data_ptr(), (const float*)weights.data_ptr(), n_pre, n_conn, n_col, is_homo); }
    else if (n_conn > 32) { int BJ = 128; dim3 grid(n_pre, (n_col + BJ - 1) / BJ); size_t shm = BJ * sizeof(float); _mm_scatter_cached_kern<<<grid, BJ, shm, s>>>((const int32_t*)indices.data_ptr(), (const float*)matrix.data_ptr(), (float*)output.data_ptr(), (const float*)weights.data_ptr(), n_pre, n_conn, n_col, is_homo); }
    else { _mm_scatter_block_kern_f32<<<n_pre, 256, 0, s>>>((const int32_t*)indices.data_ptr(), (const float*)matrix.data_ptr(), (float*)output.data_ptr(), (const float*)weights.data_ptr(), n_pre, n_conn, n_col, is_homo); }
}

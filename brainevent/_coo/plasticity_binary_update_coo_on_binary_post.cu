...
/*
 * plasticity_binary_update_coo_on_binary_post.cu
 * ===============================================
 *
 * CUDA kernels for COO post-synaptic plasticity weight updates driven by
 * binary (spike) events.
 *
 * Operation:
 *   update_coo_on_post:
 *     weight[i] += pre_trace[pre_ids[i]]  if post_spike[post_ids[i]] != 0
 *
 ...
 *
 * Python API:
 *   update_coo_on_binary_post(weight, pre_ids, post_ids, pre_trace,
 *                             post_spike, w_min=None, w_max=None,
 *                             backend=None)
 *
 * TVM FFI Entry Points (one per dtype combination):
 *   update_coo_on_post_{wt}_{spk}  where
...
DEFINE_COO_ON_POST(_f16_float, __half,         IS_ACTIVE_F16,   __half,         float,  READ_F16,  WRITE_F16)
DEFINE_COO_ON_POST(_bf16_bool, int8_t,         IS_ACTIVE_BOOL,  __nv_bfloat16,  float,  READ_BF16, WRITE_BF16)
DEFINE_COO_ON_POST(_bf16_float,__nv_bfloat16,  IS_ACTIVE_BF16,  __nv_bfloat16,  float,  READ_BF16, WRITE_BF16)

...
class XLACustomKernel:
    ...
    def benchmark_csv_output(
            ...
            Benchmark all registered backends across every configured data config.
            Same as benchmark.
            The result will out put to the csv.
            ...
            fieldnames = [
                "primitive_name", "platform", "backend", "config_name", 
                "mean_ms", "std_ms", "min_ms", "max_ms", "success", "error", "mismatch",
                "n_warmup", "n_runs"
            ]
            ...
            for config in configs:
                ...
                for be in backends_to_test:
                    ...
                    success = False
                    error_msg = None
                    mean_ms = std_ms = min_ms = float('nan')
                    output = None
                    ...
                    if verbose:
                        if success:
                            print(f"[{self.name}] [{platform}|{be}] {config.name}: mean={mean_ms:.3f}ms std={std_ms:.3f}ms min={min_ms:.3f}ms")
                        else:
                            print(f"[{self.name}] [{platform}|{be}] {config.name}: FAILED — {error_msg}")

                    record = BenchmarkRecord(
                        platform=platform, backend=be, label=config.name,
                        mean_ms=mean_ms, std_ms=std_ms, min_ms=min_ms, throughput=None,
                        success=success, error=error_msg,
                        kernel_kwargs=dict(config.kernel_kwargs), data_kwargs=dict(config.data_kwargs),
                    )
                    records.append(record)
                    ...
                    current_config_rows[be] = row_dict
                        ...
                        try:
                            if isinstance(ref_out, (list, tuple)):
                                for i, (r, o) in enumerate(zip(ref_out, other_out)):
                                    if not jnp.allclose(r, o, rtol=rtol, atol=atol):
                                        mismatch_found = True
                                        if verbose: print(f"[{self.name}][{config.name}] {ref_be} vs {other_be}: output[{i}] mismatch")
                                        break
                            else:
                                if not jnp.allclose(ref_out, other_out, rtol=rtol, atol=atol):
                                    mismatch_found = True
                                    if verbose: print(f"[{self.name}][{config.name}] {ref_be} vs {other_be}: output mismatch")
                        except Exception as cmp_exc:
                            mismatch_found = True
                            current_config_rows[other_be]["error"] = f"Comparison error: {cmp_exc}"
                        
                        if mismatch_found:
                            current_config_rows[other_be]["mismatch"] = True

                print(f"[{times}/{len_config}] {config.name} finish")

                times += 1 

                all_csv_rows.extend(current_config_rows.values())

...
"""
Benchmark for float_csrmv_p (binary CSR matrix-vector multiplication).
...
from float_unroll import csrmv_p_unroll

#from brainevent import csrmv_p
csrmv_p = csrmv_p_unroll

def generate_csr_matrix(n_rows: int, n_cols: int, density: float, seed: int):
...
def run_benchmark(
    ...
    # Generate CSR matrix
    indices, indptr, nnz = generate_csr_matrix(n_rows, n_cols, density, seed)
    ...
    # Generate weights
    if homo_weight:
        single_weight = jax.random.uniform(key_w, shape=(), dtype=jnp.float32, minval=-1.0, maxval=1.0)
        weights = jnp.full((nnz,), single_weight, dtype=jnp.float32)
    else:
        weights = jax.random.uniform(key_w, shape=(nnz,), dtype=jnp.float32, minval=-1.0, maxval=1.0)

    # Generate input vector
    if transpose:
        vector = jax.random.uniform(key_v, shape=(n_rows,), dtype=jnp.float32, minval=-1.0, maxval=1.0)
    else:
        vector = jax.random.uniform(key_v, shape=(n_cols,), dtype=jnp.float32, minval=-1.0, maxval=1.0)

    # Run benchmark
    report = csrmv_p.benchmark(
...
def main():
    ...
    parser.add_argument('--n-warmup', type=int, default=mod['warmup'], help='Number of warmup runs')
    parser.add_argument('--n-runs', type=int, default=mod['runs'], help='Number of timed runs')
    ...
    print(f"Available backends: {csrmv_p.available_backends(args.platform)}")
    ...
    for config in mod['configs']:
        ...
        for transpose in mod['transpose']:
            for homo_weight in mod['homo_weight']:

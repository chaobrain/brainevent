"""
COO Post-Synaptic Plasticity Update Benchmark
==============================================

Benchmarks all available GPU backends for ``update_coo_on_binary_post``:

    For each synapse i:
        if post_spike[post_ids[i]]:
            weight[i] += pre_trace[pre_ids[i]]

This is the STDP post-synaptic weight update for COO sparse connectivity.
...
Usage
-----
    python dev/coo/benchmark_update_coo_on_binary_post.py
    python dev/coo/benchmark_update_coo_on_binary_post.py --n_warmup 10 --n_runs 100
"""
...
from brainevent._coo.plasticity_binary import update_coo_on_binary_post_p
...
def _make_benchmark_data(*, platform):
    ...
    for n_pre, n_post, prob, density in CONFIGS:
        ...
        for bool_event in (True, False):
            ...
            nnz_spk = int((post_spike > 0).sum()) if not bool_event else int(post_spike.sum())
            name = (
            ...
            yield BenchmarkConfig(
                name=name,
                args=(weight, pre_ids, post_ids, pre_trace, post_spike),
                data_kwargs={
...
def main():
    parser = argparse.ArgumentParser(description="update_coo_on_binary_post GPU benchmark")
    ...
    print(f"update_coo_on_binary_post GPU benchmark")
    print(f"  GPU     : {gpu}")
    print(f"  warmup  : {args.n_warmup}  runs: {args.n_runs}")
    print(f"  backends: numba (CPU only), pallas, tvmffi, jax")
    print()

    update_coo_on_binary_post_p.def_benchmark_data(_make_benchmark_data)

    result = update_coo_on_binary_post_p.benchmark(
        platform='gpu',

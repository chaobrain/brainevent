...
class COO_Conn(brainstate.nn.Module):
    def __init__(
        self,
        in_size,
        out_size,
        ...
        # connections — COO format: (row, col) index arrays
        ...
        with jax.ensure_compile_time_eval():
            ...
            self.row = u.math.asarray(row, dtype=np.int32)
            self.col = u.math.asarray(col, dtype=np.int32)
            self.shape = (n_pre, n_post)

    def update(self, x) -> Union[jax.Array, u.Quantity]:
        assert x.ndim in [1, 2], 'Input must be 1D or 2D.'
        if self.data_type == 'binary':
            fn = brainevent.binary_coomv if x.ndim == 1 else brainevent.binary_coomm
        else:
            fn = brainevent.coomv if x.ndim == 1 else brainevent.coomm
        transpose = (self.efferent_target == 'post')
        return fn(self.weight.value, self.row, self.col, x, shape=self.shape, transpose=transpose)

"""
JIT Uniform Binary Event-Driven Matrix-Matrix Benchmark
=======================================================

Benchmarks all available GPU backends for ``binary_jitumm`` (gather and scatter
...
from brainevent import binary_jitumm_p, BenchmarkConfig
...
def main():
    ...
    print(f"binary_jitumm benchmark  —  GPU: {gpu}")
    print(f"warmup={args.n_warmup}  runs={args.n_runs}  backends={args.backends}")

    binary_jitumm_p.def_benchmark_data(_make_benchmark_data)

    result = binary_jitumm_p.benchmark(
        platform='gpu',

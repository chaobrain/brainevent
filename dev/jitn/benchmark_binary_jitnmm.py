"""
JIT Normal Binary Event-Driven Matrix-Matrix Benchmark
======================================================

Benchmarks all available GPU backends for ``binary_jitnmm`` (gather and scatter
modes, bool and float events) across a range of problem sizes and probabilities.

Usage
-----
    python dev/jitn/benchmark_binary_jitnmm.py
    python dev/jitn/benchmark_binary_jitnmm.py --n_warmup 5 --n_runs 50
    python dev/jitn/benchmark_binary_jitnmm.py --backends cuda_raw pallas
"""
...
from brainevent import binary_jitnmm_p, BenchmarkConfig
            ...
            for corder in (True, False):
                for bool_event in (True, False):
                    ...
                    yield BenchmarkConfig(
                        name=name,
                        args=(w_loc, w_scale, clen, B, seed),
...
def main():
    parser = argparse.ArgumentParser(description="binary_jitnmm backend benchmark")
    ...
    print(f"binary_jitnmm benchmark  —  GPU: {gpu}")
    print(f"warmup={args.n_warmup}  runs={args.n_runs}  backends={args.backends}")

    binary_jitnmm_p.def_benchmark_data(_make_benchmark_data)

    result = binary_jitnmm_p.benchmark(
        platform='gpu',

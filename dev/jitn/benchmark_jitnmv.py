"""
JIT Normal Matrix-Vector Multiplication Benchmark
=================================================

Benchmarks all available GPU backends for ``jitnmv`` (gather and scatter modes)
...
from brainevent import jitnmv_p, BenchmarkConfig
...
def main():
    ...
    print(f"jitnmv benchmark  —  GPU: {gpu}")
    print(f"warmup={args.n_warmup}  runs={args.n_runs}  backends={args.backends}")

    jitnmv_p.def_benchmark_data(_make_benchmark_data)

    result = jitnmv_p.benchmark(
        platform='gpu',

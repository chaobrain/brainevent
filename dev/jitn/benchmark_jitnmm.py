"""
JIT Normal Matrix-Matrix Multiplication Benchmark
=================================================

Benchmarks all available GPU backends for ``jitnmm`` (gather and scatter modes)
across a range of problem sizes and connection probabilities.

Usage
-----
    python dev/jitn/benchmark_jitnmm.py
    python dev/jitn/benchmark_jitnmm.py --n_warmup 5 --n_runs 50
    python dev/jitn/benchmark_jitnmm.py --backends cuda_raw pallas
"""
...
from brainevent import jitnmm_p, BenchmarkConfig
...
def main():
    parser = argparse.ArgumentParser(description="jitnmm backend benchmark")
    ...
    print(f"jitnmm benchmark  —  GPU: {gpu}")
    print(f"warmup={args.n_warmup}  runs={args.n_runs}  backends={args.backends}")

    jitnmm_p.def_benchmark_data(_make_benchmark_data)

    result = jitnmm_p.benchmark(
        platform='gpu',

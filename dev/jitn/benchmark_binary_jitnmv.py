"""
JIT Normal Binary Event-Driven Matrix-Vector Benchmark
======================================================

Benchmarks all available GPU backends for ``binary_jitnmv`` (gather and scatter
modes, bool and float events) across a range of problem sizes and probabilities.

Usage
-----
    python dev/jitn/benchmark_binary_jitnmv.py
    python dev/jitn/benchmark_binary_jitnmv.py --n_warmup 5 --n_runs 50
    python dev/jitn/benchmark_binary_jitnmv.py --backends cuda_raw pallas
"""
...
from brainevent import binary_jitnmv_p, BenchmarkConfig
            ...
            for corder in (True, False):
                for bool_event in (True, False):
                    ...
                    yield BenchmarkConfig(
                        name=name,
                        args=(w_loc, w_scale, clen, vector, seed),
...
def main():
    parser = argparse.ArgumentParser(description="binary_jitnmv backend benchmark")
    ...
    print(f"binary_jitnmv benchmark  —  GPU: {gpu}")
    print(f"warmup={args.n_warmup}  runs={args.n_runs}  backends={args.backends}")

    binary_jitnmv_p.def_benchmark_data(_make_benchmark_data)

    result = binary_jitnmv_p.benchmark(
        platform='gpu',

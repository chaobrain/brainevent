"""
CSR Matrix-Matrix Multiplication Benchmark
==========================================

Benchmarks all available backends for ``csrmm`` (float-weighted CSR SpMM,
both transpose modes, homo and hetero weights) across a range of problem
sizes and column counts.

Usage
-----
    python dev/csr/benchmark_csrmm.py
    python dev/csr/benchmark_csrmm.py --n_warmup 10 --n_runs 50
"""
...
from brainevent._csr.float import csrmm_p
...
def main():
    parser = argparse.ArgumentParser(description="csrmm backend benchmark")
    ...
    print(f"csrmm benchmark  —  GPU: {gpu}")
    print(f"warmup={args.n_warmup}  runs={args.n_runs}")
    print()

    csrmm_p.def_benchmark_data(_make_benchmark_data)

    result = csrmm_p.benchmark(
        platform='gpu',

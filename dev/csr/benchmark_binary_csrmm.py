"""
Binary CSR Matrix-Matrix Multiplication Benchmark
==================================================

Benchmarks all available backends for ``binary_csrmm`` (event-driven CSR
SpMM, homo/hetero weights, bool/float spikes) across a range of problem
sizes and column counts.

Usage
-----
    python dev/csr/benchmark_binary_csrmm.py
    python dev/csr/benchmark_binary_csrmm.py --n_warmup 10 --n_runs 50
    python dev/csr/benchmark_binary_csrmm.py --spike_rate 0.05
"""
...
from brainevent._csr.binary import binary_csrmm_p
...
def main():
    parser = argparse.ArgumentParser(description="binary_csrmm backend benchmark")
    ...
    print(f"binary_csrmm benchmark  —  GPU: {gpu}")
    ...
    binary_csrmm_p.def_benchmark_data(_data_gen)

    result = binary_csrmm_p.benchmark(
        platform='gpu',

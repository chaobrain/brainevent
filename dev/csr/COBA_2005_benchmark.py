...
class CSR_Conn(brainstate.nn.Module):
    def __init__(
        self,
        in_size,
        out_size,
        ...
        with jax.ensure_compile_time_eval():
            ...
            self.indices = u.math.asarray(flat_indices, dtype=np.int32)
            self.indptr = u.math.asarray(indptr, dtype=np.int32)
    ...
    def update(self, x) -> Union[jax.Array, u.Quantity]:
        ...
        return fn(self.weight.value, self.indices, self.indptr, x, shape=self.shape, transpose=transpose)
